...
| ✅ | **Category icon row** | **Built 2026-08-31** — `_CategoryQuickRow` under the map legend: eight shortcuts, horizontally scrollable, labels from the existing `cat_*` keys. 🚨 It is **free** while the Filter panel beside it is premium: the row narrows to one category, the panel does multi-select plus the three zone-risk toggles. That split was a judgement call and is flagged for the client — one line makes the row premium if they disagree |
| ⏳ | **Category cards with photos** | Approved 2026-08-30, **not started**. Needs a new image field in the CMS and real photos from the client before the screen is worth building. Still **not quoted** |
