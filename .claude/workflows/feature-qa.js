...
const ANGLES = [
  ...
  },
  {
    ...
    brief: [
      ...
      '   Do NOT carry a remembered version of the billing copy into the run —',
      '   this brief asserted a per-platform restore rule for eight days after',
      '   it stopped being true, and would have had reviewers restore a false',
      '   warning. Read CLAUDE.md section 4 for what the products are today,',
      '   and judge the copy against that.',
      '',
  ...
  },
  {
    ...
    brief: [
      ...
      'chore.',
      '',
      'Store and contract facts: this brief deliberately does NOT restate the',
      'product ids, types or prices. It used to, and that copy went stale',
      'within hours of a billing-model change on 2026-08-30 — leaving the',
      'file this gate actually executes asserting the opposite of the truth,',
      'so a run would have reported correct work as drift and offered to',
      'restore a retired product. Read the CLAUDE.md section 4 block headed',
      '"The plans changed" at the start of every run and treat it as the only',
      'source. Then check that every id, price and product type stated',
      'anywhere in either repo agrees with it — lib/features/premium/,',
      'firestore.rules, QA_PHASE_2B.md, DELIVERY_2B.md, the tests, this',
      'workflow and the .claude/agents briefs included.',
      'A product type is irreversible in both stores: an id created from a',
      'stale line is burned and costs a fresh store review. Treat a second',
      'statement of the product type as critical even when it agrees today.',
    ].join('\n'),
  },
]
